...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 26, 2022 at 03:16 AM
...
--
-- Dumping data for table `hodor`
--

INSERT INTO `hodor` (`id`, `users_id`, `lects_id`) VALUES
(6, 4, 6),
(4, 10, 5),
(3, 21, 6),
(5, 39, 5);
...
(58, '', 'magdy@fy.edu', 'ad', '*6BB4837EB74329105EE4568DDA7DC67ED2CA2AD9'),
(59, 'mansor', 'mans@fy.edu', 'ad', '*6BB4837EB74329105EE4568DDA7DC67ED2CA2AD9');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `students`  AS  select `users`.`id` AS `id`,`users`.`name` AS `name`,`users`.`email` AS `email`,`users`.`role` AS `role`,`users`.`pass` AS `pass` from `users` where `users`.`role` = 'stu' ;
...
ALTER TABLE `hodor`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `users_id` (`users_id`,`lects_id`);
